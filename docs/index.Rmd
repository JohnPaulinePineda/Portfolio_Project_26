---
title: "Unsupervised Learning : Estimating Outlier Scores Using Isolation Forest-Based Anomaly Detection Algorithms"
...
|      **[B.1]** 1/3 label = <span style="color: #FF0000">labs</span> variable (factor)
|             **[B.1.1]** Category 1 = <span style="color: #FF0000">Status=Outlier</span> 
|             **[B.1.2]** Category 2 = <span style="color: #FF0000">Status=Valid</span> 
|      **[B.2]** 2/3 descriptors = 2/2 numeric
...
splom(~EDA[,sapply(EDA, is.numeric)],
      ...
      cex = 1,
      alpha = 0.45,
      auto.key = list(points=TRUE, space="top", column=2),
...
## 1.5 Isolation Forest-Based Anomaly Detection

###  1.5.1. Isolation Forest (IF)
|
| [Isolation Forest](https://ieeexplore.ieee.org/document/4781136/) builds a collection of isolation trees from random subsets of data using axis-parallel splits and aggregates the anomaly decision function measurements using isolation depth averaged over a forest of such trees to come up with a final standardized outlier score for each observation. Isolation depth is the number of splits required to isolate the sample.
|
| **[A]** The IF algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**isotree**</mark> package.  
|
...
# a heatmap of the outlier scores
##################################
...
|
| [Extended Isolation Forest](https://arxiv.org/abs/1811.02141) builds a collection of isolation trees from random subsets of data using hyperplane splits and aggregates the anomaly decision function measurements using isolation depth averaged over a forest of such trees to come up with a final standardized outlier score for each observation. Isolation depth is the number of splits required to isolate the sample.
|
| **[A]** The EIF algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**isotree**</mark> package.  
|
| **[B]** The algorithm contains 3 hyperparameters:
...
|
| [Isolation Forest with Split Selection Criterion](https://link.springer.com/chapter/10.1007/978-3-642-15883-4_18) builds a collection of isolation trees from random subsets of data using hyperplane splits that maximize an averaged standard deviation gain criterion and aggregates the anomaly decision function measurements using isolation depth averaged over a forest of such trees to come up with a final standardized outlier score for each observation. Isolation depth is the number of splits required to isolate the sample.
|
| **[A]** The SCIFOREST algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**isotree**</mark> package.  
|
...
|
| [Fair-Cut Forest](https://arxiv.org/abs/2110.13402) builds a collection of isolation trees from random subsets of data using hyperplane splits that minimize the pooled sums of variances of all columns and aggregates the anomaly decision function measurements using isolation depth averaged over a forest of such trees to come up with a final standardized outlier score for each observation. Isolation depth is the number of splits required to isolate the sample. 
|
| **[A]** The FCF algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**isotree**</mark> package.  
|
...
|
| [Density Isolation Forest](https://arxiv.org/abs/2111.11639) builds a collection of isolation trees from random subsets of data using hyperplane splits and aggregates the anomaly decision function measurements using density averaged over a forest of such trees to come up with a final standardized outlier score for each observation. Density is measured as the ratio between the fraction of points in the subsample that end up in a node and the fraction of the volume in the feature space which defines the node according to the splits that lead to it. 
|
| **[A]** The DIF algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**isotree**</mark> package.  
|
| **[B]** The algorithm contains 4 hyperparameters:
|      **[B.1]** <span style="color: #FF0000">ndims</span> = number of columns to combine to produce a split held constant at a value of 2
|      **[B.2]** <span style="color: #FF0000">ntrees</span> = number of binary trees to build for the model held constant at a value of 100.
...
###  1.5.6. Boxed Isolation Forest (BIF)
|
| [Boxed Isolation Forest](https://arxiv.org/abs/2111.11639) builds a collection of isolation trees from random subsets of data using hyperplane splits and aggregates the anomaly decision function measurements using boxed ratio averaged over a forest of such trees to come up with a final standardized outlier score for each observation. Density is measured as the ratio between the volume of the boxed feature space for the node as defined by the smallest and largest values from the split conditions for each column (bounded by the variable ranges in the sample) and the variable ranges in the tree sample. 
|
| **[A]** The BIF algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**isotree**</mark> package.  
|
| **[B]** The algorithm contains 4 hyperparameters:
|      **[B.1]** <span style="color: #FF0000">ndims</span> = number of columns to combine to produce a split held constant at a value of 2
|      **[B.2]** <span style="color: #FF0000">ntrees</span> = number of binary trees to build for the model held constant at a value of 100.
|      **[B.3]** <span style="color: #FF0000">missing_action</span> = method of handling missing data at both fitting and prediction time held constant at a value of "FAIL".
|      **[B.4]** <span style="color: #FF0000">scoring_metric</span> = metric to use for Determining outlier scores held constant at a value of "BOXED_RATIO".
|
| **[C]** Using the label information from the <span style="color: #FF0000">Status</span> variable defined prior to the analysis, the algorithm was able to sufficiently discriminate between <span style="color: #FF0000">Status=Outlier</span> and <span style="color: #FF0000">Status=Valid</span>, as demonstrated by their comparably expressed deviance score densities.
|      **[C.1]** ROC Curve AUC = 0.57868
|
|
...
##################################
# Implementing the BIF Algorithm
##################################
OD_BIF <- isolation.forest(OD, 
                          ndim=2, 
                          ntrees=100, 
                          missing_action="fail",
...
# applied to the entire space range
##################################
OD_BIF_DevianceScores <- predict(OD_BIF, Space_Range)
...
# applied to the dataset
##################################
OD_BIF_PredictedScores <- predict(OD_BIF, OD)

##################################
# Exploring the outlier scores
# between the valid and outlier points
##################################
max(OD_BIF_PredictedScores)
min(OD_BIF_PredictedScores)

OD_BIF_Summary <- DR_PCA_SUBSET

OD_BIF_Summary$Scores <- OD_BIF_PredictedScores
OD_BIF_Summary$Label <- rep("BIF", nrow(OD_BIF_Summary))
...
densityplot( ~ Scores | Label,
            ...
            xlab = "Outlier Scores",
            ylab = "Density",
            auto.key = list(columns=(length(levels(OD_BIF_Summary$Status)))))

##################################
# Evaluating the apparent 
# discrimination performance
# of the BIF Algorithm
##################################
Status <- ifelse(OD_BIF_Summary$Status=="Valid",0,1)

(OD_BIF_ROCCurveAUC <- AUC(OD_BIF_PredictedScores, Status))
...
|
| **[A]** The isolation forest-based anomaly detection algorithms applied to the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors which were able to sufficiently capture the outlier points based on the higher estimated ROC curve AUC are the following :
|      **[A.1]** **FCF : Fair-Cut Forest** (<mark style="background-color: #CCECFF">**isotree**</mark> package)
|             **[A.1.1]** ROC Curve AUC = 0.78291
|      **[A.2]** **DIF: Density Isolation Forest** (<mark style="background-color: #CCECFF">**isotree**</mark> package)
|             **[A.2.1]** ROC Curve AUC = 0.77588
|
|
...
par(mfrow=c(3,2),
    mar = c(2.5,2.2,2,2.5))
Plot_Space_Range(OD_IF_DevianceScores,"IF",1.0)
Plot_Space_Range(OD_EIF_DevianceScores,"EIF",1.0)
Plot_Space_Range(OD_SCIFOREST_DevianceScores,"SCIFOREST",1.0)
Plot_Space_Range(OD_FCF_DevianceScores,"FCF",1.0)
Plot_Space_Range(OD_DIF_DevianceScores,"DIF",1.0)
Plot_Space_Range(OD_BIF_DevianceScores,"BIF",1.0)
mtext("Outlier Scores",
      side = 3,
      line = -1.25,
      outer = TRUE)
...
(OutlierDetectionPeformance_Summary_Plot <- dotplot(OutlierDetectionAlgorithm ~ OutlierDetectionMetrics,
                          ...
                          auto.key = list(adj=1, space="top", column=2),
                          type=c("p", "h"),
                          origin = 0,
                          alpha = 0.45,
                          pch = 16,
...
|
| ![](D:/Github_Codes/ProjectPortfolio/Portfolio_Project_26/images/Project26_Summary.png)
|
# **3. References**
|
...
| **[Article]** [An Introduction to Isolation Forests](https://cran.r-project.org/web/packages/isotree/vignettes/An_Introduction_to_Isolation_Forests.html) by David Cortes
| **[Article]** [Isolation Forest: The Art of Cutting Off From the World](https://quantdare.com/isolation-forest-algorithm/) by QuantDare Team
| **[Article]** [Anomaly Detection with Isolation Forest and Kernel Density Estimation](https://machinelearningmastery.com/anomaly-detection-with-isolation-forest-and-kernel-density-estimation/) by Muhammad Asad Iqbal Khan
| **[Article]** [Isolation Forest Algorithm for Anomaly Detection](https://arpitbhayani.me/blogs/isolation-forest) by Arpit Bhayani
| **[Article]** [Isolation Forest Example](https://scikit-learn.org/stable/auto_examples/ensemble/plot_isolation_forest.html) by Scikit-Learn Team
| **[Article]** [Isolation Forest for Anomaly Detection ](https://www.mathworks.com/help/stats/isolationforest.html) by Mathworks Team
| **[Article]** [Machine Learning Interpretability For Isolation Forest Using SHAP](https://www.data4v.com/machine-learning-interpretability-for-isolation-forest-using-shap/) by Prasad Kulkarni
| **[Article]** [What is Isolation Forest?](https://dsworld.org/what-is-an-isolation-forest/) by Data Science World Team
| **[Article]** [Anomaly Detection: Isolation Forest Agorithm](https://majdarbash.github.io/aws-cmls/2019-11-21-isolation-forest-algorithm/) by Maj Darbash
...
| **[Publication]** [Isolation-Based Anomaly Detection](https://dl.acm.org/doi/10.1145/2133360.2133363) by Fei Tony Liu, Kai Ming Ting and Zhi-Hua Zhou (ACM Transactions on Knowledge Discovery from Data)
| **[Publication]** [On Detecting Clustered Anomalies using SCiForest](https://link.springer.com/chapter/10.1007/978-3-642-15883-4_18) by Fei Tony Liu and Kai Ming Ting (Joint European Conference on Machine Learning and Knowledge Discovery in Databases)
| **[Publication]** [Extended Isolation Forest](https://arxiv.org/abs/1811.02141) by Sahand Hariri, Matias Carrasco Kind and Robert Brunner (ARXIV)
| **[Publication]** [Revisiting Randomized Choices in Isolation Forests](https://arxiv.org/abs/2110.13402) by David Cortes (ARXIV)
| **[Publication]** [Isolation Forests: Looking Beyond Tree Depth](https://arxiv.org/abs/2111.11639) by David Cortes (ARXIV)
